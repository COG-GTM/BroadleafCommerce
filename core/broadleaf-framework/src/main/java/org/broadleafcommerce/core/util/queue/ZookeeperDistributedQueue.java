...
public class ZookeeperDistributedQueue<T extends Serializable> implements DistributedBlockingQueue<T> {
    ...
    /**
     * Name of the system property that can be used to replace {@link #DEFAULT_DESERIALIZATION_FILTER_PATTERN}.  The value must be a valid
     * pattern for {@link ObjectInputFilter.Config#createFilter(String)}.
     */
    public static final String DESERIALIZATION_FILTER_PROPERTY = "broadleaf.zookeeper.queue.deserialization.filter";

    /**
     * Allow list applied to every object read from Zookeeper.  Anything that is not explicitly allowed is rejected, which prevents
     * arbitrary gadget classes on the classpath from being instantiated during deserialization (CWE-502).
     */
    ...
    /**
     * Creates the {@link ObjectInputFilter} that restricts the types that may be deserialized from the queue.  Subclasses may override
     * this to further restrict, or to widen, the set of allowed types.
     *
     * @return
     */
    protected ObjectInputFilter createDeserializationFilter() {
        String pattern = System.getProperty(DESERIALIZATION_FILTER_PROPERTY, DEFAULT_DESERIALIZATION_FILTER_PATTERN);
     ...
     * allowed by {@link #createDeserializationFilter()}.
     *
     * @param bytes
     * @return
     */
    protected Object deserialize(byte[] bytes) {
        ...
        try {
            ...
            throw new DistributedQueueException("An element from the Zookeeper queue was rejected because its type is not allowed by the "
                    + "deserialization filter for queue: " + getQueueFolderPath(), e);
