...
public class OrderServiceImpl implements OrderService {
    ...
    /**
     * Determines whether the given order item belongs to the order identified by the given id. The order id is
     * resolved by the caller from its own security context (for example the current cart or a named order looked up
     * for the current customer), so this comparison is what keeps an item id supplied by an end user from targeting
     ...
     * @return true if the item is not null and is owned by the given order
     */
